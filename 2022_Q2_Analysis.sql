...
SELECT 
        ...
        SUM(
            CASE WHEN ride_id IS NOT NULL AND start_station_name IS NOT NULL THEN 1 ELSE 0 END
            ) AS total,
        SUM(
            CASE WHEN member_casual = 'member' AND start_station_name IS NOT NULL THEN 1 ELSE 0 END
            ) AS member,
        SUM(
            CASE WHEN member_casual = 'casual' AND start_station_name IS NOT NULL THEN 1 ELSE 0 END
            ) AS casual
FROM 
        `divvy-tripdata-426901.2022_Tripdata_Q2.2022_Tripdata_Q2`
GROUP BY 
